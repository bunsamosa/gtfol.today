<script setup lang="ts">
	...
	function openDemo() {
		window.open(project.project_demo_url, '_blank');
	}
</script>

<div class="flex h-full">
	<div class="m-4 w-80 card variant-ghost rounded-lg shadow-lg flex flex-col justify-between">
		{#await updateParams()}
			<div class="placeholder" />
		{:then _}
			<header class="flex flex-col card-header">
				<!-- Rank and Badge -->
			...
			<div class="flex flex-col m-4">
				<button class="btn variant-ghost-secondary rounded-xl m-2 p-2" on:click={openDemo}
					>Demo</button
				>
			</div>
		{/await}
